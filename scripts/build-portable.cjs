...
function writePortableReadme(destDir, exeName) {
  const content = [
    "RNAmeta Portable",
    "",
    `1. Run ${exeName} from this folder.`,
    "2. Keep the resources folder next to the executable.",
    "3. The app still requires Microsoft WebView2 Runtime on the target machine.",
    "",
    "This folder was generated by scripts/build-portable.cjs."
  ].join("\r\n");

  fs.writeFileSync(path.join(destDir, "README-Portable.txt"), content, "utf8");
}
...
writePortableReadme(portableDir, portableExeName);
