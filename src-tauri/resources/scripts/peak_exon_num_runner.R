...
if (length(args) < 2) {
  stop("Usage: peak_exon_num_runner.R <request.json> <response.json>")
}
...
source(file.path(script_root, "rna", "peak_exon_num.functions.R"))
source(file.path(script_root, "rna", "peak_exon_num.data.R"))
source(file.path(script_root, "rna", "peak_exon_num.chart.R"))
source(file.path(script_root, "rna", "peak_exon_num.analysis.R"))

peak_exon_num_progress_update <- function(progress, value, detail) {
  message(sprintf("[peak-exon-num][%s%%] %s", value, detail))
}
...
tryCatch(
  {
    ...
    result <- run_peak_exon_num_analysis(
