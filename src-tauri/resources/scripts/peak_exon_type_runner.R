...
if (length(args) < 2) {
  stop("Usage: peak_exon_type_runner.R <request.json> <response.json>")
}
...
source(file.path(script_root, "rna", "peak_exon_type.functions.R"))
source(file.path(script_root, "rna", "peak_exon_type.data.R"))
source(file.path(script_root, "rna", "peak_exon_type.chart.R"))
source(file.path(script_root, "rna", "peak_exon_type.analysis.R"))

peak_exon_type_progress_update <- function(progress, value, detail) {
  message(sprintf("[peak-exon-type][%s%%] %s", value, detail))
}
...
tryCatch(
  {
    ...
    result <- run_peak_exon_type_analysis(
